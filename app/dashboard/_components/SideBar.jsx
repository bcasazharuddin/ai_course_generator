...
function SideBar() {
  ...
  return (
    <div className='fixed h-full md:w-64 p-5 shadow-md'>
        ...
        <Link href='/'>
        <Image src={'/logo.png'} width={50} height={50}     className="object-cover" /></Link>
        </div>
        <hr className = 'my-5'/>
        <ul>
            {Menu.map((item,index)=>(
                <Link href={item.path}>
                <div className={`flex items-center gap-2 text-gray-600 p-3 cursor-pointer hover:bg:gray-100 hover:bg-gray-100 hover:text-black rounded-lg mb-3 ${item.path==path && 'bg-gray-100 text-black'}`}>
